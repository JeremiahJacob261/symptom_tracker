...
async function fetchEntries(
  ...
  const params = new URLSearchParams({
    select:
      "id,client_id,pain_level,body_area,mood,notes,symptoms,custom_symptoms,model_details,temperature_celsius,occurred_at,updated_at,deleted_at",
    deleted_at: "is.null",
  ...
  const endpoint = "https://pggvcuchcrytifxnzhef.supabase.co/rest/v1/mar_symptom_entries";
  const headers = {
    apikey: "sb_publishable_HIltu5fP_Y4YU-mhgABncg_wlmIu5jx",
    authorization: `Bearer ${token}`,
  };
  let response = await fetch(`${endpoint}?${params}`, { headers });
  // Keep existing insight requests working during rollout if the app update is
  // deployed before the additive Supabase migration.
  if (response.status === 400) {
    ...
    response = await fetch(`${endpoint}?${params}`, { headers });
  }
  if (!response.ok) {
...
function redFlags(entries: SymptomEntry[]) {
  ...
  for (const entry of entries) {
    ...
    const symptoms = [
      ...
      ...Object.values(entry.model_details ?? {}),
    ]
      .join(" ")
      .toLowerCase();
...
function symptomFrequency(entries: SymptomEntry[]) {
  return entries.reduce<Record<string, number>>((counts, entry) => {
    ...
    for (const detail of Object.values(entry.model_details ?? {})) {
      if (!detail) continue;
...
async function runInsightModel(
  ...
  const sampleEntries = entries.slice(0, 30).map((entry) => ({
    ...
    model_details: entry.model_details,

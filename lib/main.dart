...
class _HomeScreenState extends State<HomeScreen> {
  ...
  final Map<String, TextEditingController> _detailControllers = {
    for (final prompt in SymptomModelInput.prompts)
      prompt.id: TextEditingController(),
  };
  ...
  Future<void> _saveEntry() async {
    ...
    final shouldRunPrediction = _selectedSymptoms.isNotEmpty ||
        _customSymptomsController.text.trim().isNotEmpty ||
    ...
    final entry = <String, dynamic>{
      ...
      'model_details_json': jsonEncode(modelDetails),
    ...
    final symptomText = _buildSymptomText(entry);
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Could not save this entry. Try again.')),
      );
      return;
    }
    ...
    if (!DiseasePredictionService.isConfigured || !shouldRunPrediction) {
      setState(() => _isSaving = false);
      _showSavedMessage();
      return;
    }

    final service = DiseasePredictionService.fromEnvironment();
    try {
      final prediction = await service.predict(
        symptomText: symptomText,
        painLevel: entry['pain_level'] as int,
        temperatureCelsius: temperatureCelsius,
      );
      if (!mounted) return;
      setState(() => _isSaving = false);
      await _showPredictionResult(prediction);
    } on DiseasePredictionException catch (error) {
      if (!mounted) return;
      setState(() => _isSaving = false);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Entry saved. ${error.message}'),
  ...
  String _buildSymptomText(Map<String, dynamic> entry) {
    return SymptomModelInput.build(
      symptoms: _selectedSymptoms,
      bodyArea: entry['body_area']?.toString(),
      customSymptoms: entry['custom_symptoms']?.toString(),
      notes: entry['notes']?.toString(),
      details: readModelDetails(entry),
      temperatureCelsius: readTemperatureCelsius(entry),
    );
  }
  ...
  Future<void> _showPredictionResult(DiseasePredictionResult result) {
    final theme = Theme.of(context);
    final isUrgent = result.urgent;
    final confidence =
        (result.confidence * 100).clamp(0, 100).toStringAsFixed(0);
    final closestPattern = result.closestPattern;

    return showDialog<void>(
      context: context,
      barrierDismissible: !isUrgent,
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              if (isUrgent) ...[
                const Text(
                  'The safety screening found symptoms that can need urgent assessment. '
                  'Contact local emergency services or go to the nearest emergency department if symptoms are severe, sudden, or worsening.',
                ),
                const SizedBox(height: 12),
                ...result.urgentFlags.map((flag) => Text('• $flag')),
              ] else if (result.lowConfidence) ...[
                Text(
                  closestPattern == null
                      ? 'The ${result.model} model did not return a pattern comparison.'
                      : 'The closest pattern found was $closestPattern '
                          '($confidence%). This is below the required 60% confidence threshold, '
                          'so it is shown as a low-confidence comparison—not a reliable match.',
                ...
                if (result.alternatives.isNotEmpty) ...[
                  ...
                  ...result.alternatives.take(3).map(
                        (alternative) => Text(
                          '• ${alternative.condition}: '
                          '${(alternative.probability * 100).clamp(0, 100).toStringAsFixed(0)}%',
                        ),
                      ),
                ],
                const SizedBox(height: 12),
                const Text(
                  'Add more specific symptom details or speak with a qualified healthcare professional.',
                ),
              ] else ...[
                Text(
                  'Your description is most similar to patterns associated with '
                  '${result.possibleMatch} ($confidence% confidence).',
                  style: const TextStyle(fontWeight: FontWeight.w700),
                ),
                if (result.alternatives.isNotEmpty) ...[
                  const SizedBox(height: 16),
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(
                    ...
                    hintText:
                        'Describe specific symptoms, e.g. burning urination or scaly patches',
                  ),
                ),
                if (SymptomModelInput.promptsFor(_selectedSymptoms)
                    .isNotEmpty) ...[
                  const SizedBox(height: 18),
                  Text(
                    'Optional details for better pattern matching',
                  ...
                  ...SymptomModelInput.promptsFor(_selectedSymptoms).map(
                    (prompt) => Padding(
                      padding: const EdgeInsets.only(bottom: 12),
                      child: TextField(
                        controller: _detailControllers[prompt.id],
                        textCapitalization: TextCapitalization.sentences,
        ...
        SliverToBoxAdapter(
          child: Padding(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text(_isSaving ? 'Saving & analyzing...' : 'Save Entry'),
            ),
          ),
        ),
      ],
...
class _HistoryScreenState extends State<HistoryScreen> {
    ...
    setState(() {
      if (query.isEmpty) {
        ...
        _filteredEntries = _entries.where((entry) {
          ...
          final modelDetails = readModelDetails(entry).values.join(' ');
          final temperature = readTemperatureCelsius(entry)?.toString() ?? '';
          final text = ('${entry['body_area']} ${entry['mood']} '
                  '${entry['notes']} $symptoms $customSymptoms $modelDetails $temperature')
              .toLowerCase();
  ...
  Future<void> _showEntryDetails(Map<String, dynamic> entry) async {
    ...
    final customSymptoms = readCustomSymptoms(entry);
    final modelDetails = readModelDetails(entry);
  ...
  Future<void> _exportCSV() async {
    ...
    final csvData = [
      ...
      ...entries.map((e) => [
            ...
            readModelDetails(e).values.join('; '),
            e['notes'] ?? '',
          ]),
    ];

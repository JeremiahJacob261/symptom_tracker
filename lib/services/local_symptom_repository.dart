...
class LocalSymptomRepository {
  ...
  Future<Database> _initDB() async {
    ...
    return openDatabase(
      ...
      onUpgrade: (db, oldVersion, newVersion) async {
        ...
        if (oldVersion < 4) {
          await _addEntryModelDetailsColumn(db);
        }
      },
    );
  }
  ...
  Future<void> _addEntryModelDetailsColumn(Database db) async {
    final existing = await db.rawQuery('PRAGMA table_info(entries)');
    final names = existing.map((row) => row['name'] as String).toSet();
    if (!names.contains('model_details_json')) {
      await db
          .execute('ALTER TABLE entries ADD COLUMN model_details_json TEXT');
    }
  }
  ...
  Future<void> upsertRemoteEntry(Map<String, dynamic> remote) async {
    await _upsertRemote('entries', remote, {
      ...
      'model_details_json': jsonEncode(remote['model_details'] ?? const {}),
